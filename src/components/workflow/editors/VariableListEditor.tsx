...
interface VariableListEditorProps {
  value: VariableRow[] | null | undefined;
  onChange: (v: VariableRow[]) => void;
...
export default function VariableListEditor({
  ...
  valuePlaceholder = 'Value or {{$json.field}}',
  addButtonLabel = 'Add Variable',
  disabled = false,
}: VariableListEditorProps) {
  const rows: VariableRow[] = Array.isArray(value) ? value : [];
  ...
  const handleNameChange = useCallback(
    (index: number, name: string) => {
      onChange(rows.map((r, i) => (i === index ? { ...r, name } : r)));
    },
    [rows, onChange]
  );
      ...
      {rows.length > 0 && (
        <div className="space-y-1">
          <div className="flex gap-1.5 px-0.5">
            <span className="text-[10px] text-muted-foreground/70 flex-1">Name</span>
            <span className="text-[10px] text-muted-foreground/70 flex-1">Value</span>
            <span className="w-7" />
          </div>
          {rows.map((row, i) => (
