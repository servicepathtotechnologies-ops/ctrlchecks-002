...
export interface LightPillarProps {
  ...
  /**
   * When true, clears the canvas to transparent and uses additive blending so the pillar
   * glows on top of a light page. When false, opaque clear + normal blend (for CSS screen on dark).
   */
  compositeForLightBackground?: boolean;
}
...
export default function LightPillar({
  ...
  pillarRotation = 0,
  quality = "high",
  compositeForLightBackground = false,
}: LightPillarProps) {
  ...
  useEffect(() => {
    ...
    renderer.setClearColor(0x000000, compositeForLightBackground ? 0 : 1);
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        float alpha = 1.0;
        // Light pages: alpha from energy so empty rays stay transparent on white (additive on white was invisible).
        if (uLuminanceAlpha > 0.5) {
          float l = length(max(rgb, vec3(0.0)));
          alpha = clamp(pow(min(l, 4.0) / 4.0, 0.36) * 3.0, 0.0, 1.0);
        }
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uLuminanceAlpha: { value: compositeForLightBackground ? 1.0 : 0.0 },
      },
      transparent: true,
      depthWrite: false,
      depthTest: false,
  ...
  useEffect(() => {
    ...
    rendererRef.current.setClearColor(0x000000, compositeForLightBackground ? 0 : 1);
    materialRef.current.uniforms.uLuminanceAlpha.value = compositeForLightBackground ? 1.0 : 0.0;
    materialRef.current.needsUpdate = true;
  }, [compositeForLightBackground]);
